...
File.open('Surviving_Date_Filter_BFC.txt').each { |line| bug_fixing_commits << line }
...
for each in counting_array
  ...
  if (!each.nil? && !each.empty?)
    ...
    if yes_comment_ghost
      #ghost_commits += 1 - DO NOTHING, WE'RE NOT COUNTING GHOST COMMITS ANYMORE
    else
      surviving_Comments_Filter_BFC << link_counting_to_bfc[index]
    end
...
File.open('Surviving_Comments_Unique_Implic_Commits.txt', 'w+') do |f1|
  f1.puts(surviving_comments_unique_implic_commits)
end

File.open('Surviving_Comments_Filter_BFC.txt', 'w+') do |f2|
  f2.puts(surviving_Comments_Filter_BFC)
end
