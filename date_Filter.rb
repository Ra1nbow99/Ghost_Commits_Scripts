...
for each in counting_array
  ...
  if (!each.nil? && !each.empty?)
    ...
    if yes_ghost
      ...
      surviving_Date_Filter_BFC << link_counting_to_bfc[index]
    end
...
File.open('Unique_Implic_Commits.txt', 'w+') do |f|
  f.puts(all_unique_implic_commits)
end

File.open('Surviving_Date_Unique_Implic_Commits.txt', 'w+') do |f1|
  f1.puts(surviving_date_unique_implic_commits)
end

File.open('Surviving_Date_Filter_BFC.txt', 'w+') do |f2|
  f2.puts(surviving_Date_Filter_BFC)
end
...
puts "All Surviving Unique Implic Commits: #{surviving_date_unique_implic_commit_count}"
after_Filter_Ratio = ((surviving_date_unique_implic_commit_count.to_f)/total_commits)*100
