...
File.open('Surviving_Whitespace_Filter_BFC.txt').each { |line| bug_fixing_commits << line }
...
#EACH ENTRY IN THIS ARRAY WILL ALSO HAVE SAME INDEDX AS COUNTING_ARRAY AND WILL CONTAIN SIZE OF COMMIT
commit_size = []
      ...
      for result in results 
          ...
          counting_array[index] = (counting_array[index]).to_s + m.to_s + " "
           ...
           #puts("FARIDA")
            #puts n
          #ends
          result_size = result.last - result.first
          if (result_size == 0)
            result_size += 1
          end
          if commit_size[index].nil?
            commit_size[index] = 0
          end
          commit_size[index] += result_size
      end
    end
  end 
...
surviving_size_unique_implic_commits = []
  ...
  if (!each.nil? && !each.empty?)
    ...
    splitLines.each do |line| 
      ...
      implic_Code = implic_Code.to_s + " "
        ...
        # SIZE CHECK
        # INSERT LOGIC TO CHECK COMMIT SIZE AT INDEX - IF STATEMENT
        if !(commit_size[index] > 1000)
          yes_size_ghost = false
          if (surviving_date_unique_implic_commits.include?(impl1) &&
             surviving_comments_unique_implic_commits.include?(impl1) &&
             surviving_whitespace_unique_implic_commits.include?(impl1) &&
             !surviving_size_unique_implic_commits.include?(impl1))
                  surviving_size_unique_implic_commits << impl1
          end          
        end
    end
    ...
    #end
    if !yes_size_ghost
      surviving_Size_Filter_BFC << link_counting_to_bfc[index]
    end
  end
  index+=1
end
...
surviving_size_unique_implic_commits_count = surviving_size_unique_implic_commits.length
...
#File.open('Filters_Output.txt', 'a+') do |f3|
  #f3.puts " "
  #f3.puts " "
  #f3.puts " "
  #f3.puts "------ WHITESPACE FILTER OUTPUT ------"
  ##f3.puts "All Unique Implic Commits Surviving Comments Filter: #{surviving_comments_unique_implic_commit_count}"
  ##after_Comments_Filter_Ratio = ((surviving_comments_unique_implic_commit_count.to_f)/total_commits)*100
  #f3.puts "All Surviving COMMENTS FILTER Implic Commits Ratio: #{after_Comments_Filter_Ratio}"
  #f3.puts "All Unique Implic Commits Surviving Whitespace Filter: #{surviving_whitespace_unique_implic_commit_count}"
  #after_Whitespace_Filter_Ratio = ((surviving_whitespace_unique_implic_commit_count.to_f)/total_commits)*100
  #f3.puts "All Surviving WHITESPACE FILTER Implic Commits Ratio: #{after_Whitespace_Filter_Ratio}"
#end
...
#File.open('Size_Check.txt', 'w+') do |f4|
  #f4.puts commit_size
#end


File.open('Surviving_Size_Unique_Implic_Commits.txt', 'w+') do |f1|
  f1.puts(surviving_size_unique_implic_commits)
end

File.open('Surviving_Size_Filter_BFC.txt', 'w+') do |f2|
  f2.puts(surviving_Size_Filter_BFC)
end

File.open('Size_Filter_Output.txt', 'a+') do |f3|
  f3.puts "------ SIZE FILTER OUTPUT ------"
  f3.puts "All Unique Implic Commits Surviving Size Filter: #{surviving_size_unique_implic_commits_count}"
  after_Size_Filter_Ratio = ((surviving_size_unique_implic_commits_count.to_f)/total_commits)*100
  f3.puts "All Surviving SIZE FILTER Implic Commits Ratio: #{after_Size_Filter_Ratio}"
end
